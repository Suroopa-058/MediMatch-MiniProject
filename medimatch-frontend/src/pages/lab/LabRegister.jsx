...
import { labRegister } from '../../services/labAuthService';
...
export default function LabRegister() {
  ...
  const handleSubmit = async () => {
    ...
    setLoading(true);
    try {
      const result = await labRegister({
        name: form.name,
        email: form.email,
        phone: form.phone,
    ...
    <div className="min-h-screen flex">
      <div className="w-5/12 bg-gradient-to-br from-indigo-600 to-indigo-800 flex flex-col items-center justify-center p-12 text-white">
        ...
        <p className="text-indigo-100 text-center text-sm mb-8 max-w-xs">
          Join our network and start uploading patient reports securely.
        </p>
      ...
      <div className="flex-1 flex items-center justify-center p-12 bg-white">
        <div className="w-full max-w-md">
          <button onClick={() => navigate('/role')} className="text-indigo-600 text-sm font-bold mb-6">← Change Role</button>

          <div className="text-2xl mb-1">🧪</div>
          <h2 className="text-2xl font-bold text-gray-800 mb-1">Lab Registration</h2>
          <p className="text-gray-400 text-sm mb-6">Create your lab account</p>

          {error && (
            <div className="bg-red-50 border border-red-200 text-red-600 text-sm rounded-xl px-4 py-3 mb-4">⚠️ {error}</div>
          )}

          <label className="block text-xs font-bold text-gray-500 mb-1">Lab Name</label>
          ...
          <p className="text-center text-xs text-gray-400 mt-4">
            ...
            <span onClick={() => navigate('/lab/login')} className="text-indigo-600 font-bold cursor-pointer">Login</span>
          </p>
        </div>
      </div>
    </div>
