...
export default function DoctorVideoConsult() {
  ...
  const joinChannel = async () => {
    try {
      ...
      agoraClient.on('user-unpublished', () => setRemoteUser(null));
  ...
  const cleanup = async (nav = true) => {
    ...
    if (nav) navigate('/doctor/dashboard');
  };
  ...
  const sendMessage = async () => {
    ...
    try {
      await axios.post('https://medimatch-backend-4t7f.onrender.com/api/video/message',
        { appointmentId, message: text, senderRole: 'doctor' },
        { headers: { Authorization: `Bearer ${localStorage.getItem('token')}` } }
      );
    } catch (e) { console.error(e); }
  };
  ...
  const handleEndCall = async () => {
    if (hasEndedRef.current) return;
    hasEndedRef.current = true;
    socketRef.current?.emit('end-call', { appointmentId, endedBy: 'doctor' });
    setCallActive(false);
    await cleanup(false);
    setTimeout(() => goToPrescription(), 2000);
  };
    ...
    <div className="h-screen bg-gray-900 flex flex-col overflow-hidden">
      ...
      <div className="flex items-center justify-between px-6 py-3 bg-gray-800 border-b border-gray-700">
        ...
        <div className="flex items-center gap-2">
          ...
          <span className="text-gray-300 text-sm">{user.name || 'Dr.'}</span>
        </div>
      </div>
        ...
        <div className="flex-1 flex items-center justify-center">
          <div className="text-center">
            ...
            <h3 className="text-white text-xl font-bold mb-2">
              {endedByOther ? 'Patient ended the call' : 'Call Ended'}
            </h3>
            <p className="text-gray-400 text-sm">Duration: {formatTime(callTime)}</p>
            <p className="text-gray-400 text-sm mt-1">Redirecting to prescription...</p>
          </div>
        </div>
      ) : (
          ...
          <div className="flex-1 flex flex-col relative">
            <div className="flex-1 relative bg-gray-800 flex items-center justify-center">
              ...
              {!remoteUser && (
                <div className="relative z-20 text-center">
                  <div className="w-32 h-32 bg-blue-700 rounded-full flex items-center justify-center text-6xl mx-auto mb-4 shadow-2xl">👩</div>
                  <h3 className="text-white text-xl font-bold">Waiting for patient...</h3>
              ...
              {patientReport && (
                <div className="absolute top-4 right-4 z-20 bg-gray-800 bg-opacity-90 rounded-xl p-3 text-xs text-white w-48">
                  ...
                  {patientReport.glucose && <div className="text-red-400">🔴 Glucose: {patientReport.glucose}</div>}
                  {patientReport.hemoglobin && <div className="text-orange-400">🟡 Hemoglobin: {patientReport.hemoglobin}</div>}
                  {patientReport.cholesterol && <div className="text-green-400">🟢 Cholesterol: {patientReport.cholesterol}</div>}
                </div>
              )}
              ...
              {muted && (
                <div className="absolute top-4 left-4 z-20 bg-red-500 bg-opacity-90 text-white text-xs px-3 py-1.5 rounded-full">
                  🔇 You are muted
                </div>
              )}
            </div>
            ...
            <div className="bg-gray-800 px-8 py-4 flex items-center justify-center gap-4 border-t border-gray-700">
              ...
              <button onClick={goToPrescription}
                className="flex flex-col items-center gap-1 w-16 h-16 rounded-2xl bg-blue-600 hover:bg-blue-700 flex items-center justify-center transition-all">
                <span className="text-xl">💊</span>
                <span className="text-white text-xs">Prescribe</span>
              </button>
            </div>
          </div>
          ...
          {chatOpen && (
            <div className="w-80 bg-gray-800 border-l border-gray-700 flex flex-col">
              ...
              <div className="flex-1 overflow-y-auto p-4 space-y-3">
                {messages.map((msg, i) => (
                  <div key={i} className={`flex ${msg.from === 'doctor' ? 'justify-start' : 'justify-end'}`}>
                    <div className={`max-w-48 rounded-2xl px-3 py-2 ${msg.from === 'doctor' ? 'bg-blue-600 text-white rounded-bl-sm' : 'bg-gray-700 text-gray-100 rounded-br-sm'}`}>
